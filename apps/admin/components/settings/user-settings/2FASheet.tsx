...
export const TwoFASheet = ({ hasTwoFa }: {
  hasTwoFa: boolean
}) => {
  ...
  useEffect(()=>{
    ...
    if (!open && formState.isDirty) {
      addAlert({
        title: '2FA',
        description: 'Are you sure you want to close this sheet? 2FA is not enabled yet',
        cancelText: 'Cancel',
        actionText: 'Close',
        onDecision: (cancel) => {
          if (!cancel) {
        ...
        toast({
          ...
          description: (
            <div className={'flex flex-row items-center space-x-2.5'}>
              ...
              <p className='text-sm text-foreground'>Successfully disabled Two Factor Authentication!</p>
            </div>
          ),
        });
  ...
  function onSubmit(data: z.infer<typeof FormSchema>) {
    verifyQrCodeRequest(data.code).then((res)=>{
      setOpen(false)
      toast({
        title: '2FA',
        description: (
  ...
  return <Sheet open={open} onOpenChange={(open)=> setOpen(open)}>
    <Switch onClick={() => handleToggle()} checked={switchValue} defaultValue={switchValue.toString()} value={switchValue.toString()}/>
