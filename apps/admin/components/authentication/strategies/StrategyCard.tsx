  ...
  return (
    <Card className={'col-span-2 px-0 '} key={`${strategy.name}`}>
      ...
      <CardContent
        className={cn(
          'flex flex-row',
          strategy.oauth ? 'justify-between' : 'justify-start gap-x-6'
        )}
      >
        Supports:
        <ul>
          ...
          {strategy.supports.register && (
            <li
              className={'flex flex-row items-center'}
              key={`${strategy.name}-register`}
            >
              <CheckIcon className={'w-4 h-4 mr-2'} /> Register
            </li>
          )}
        </ul>
        {strategy.oauth && (
          <>
            ...
            <ul>
              {strategy.oauth?.redirect && (
                <li
                  className={'flex flex-row items-center'}
                  key={`${strategy.name}-redirect`}
                >
                  <CheckIcon className={'w-4 h-4 mr-2'} /> Redirect
                </li>
              )}
              {strategy.oauth?.native && (
                <li
                  className={'flex flex-row items-center'}
                  key={`${strategy.name}-native`}
                >
                  <CheckIcon className={'w-4 h-4 mr-2'} /> Native
                </li>
              )}
            </ul>
          </>
