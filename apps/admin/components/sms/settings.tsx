...
const FormSchema = z.object({
  ...
  if (schema.providerName === 'awsSns' && !isEmpty(schema.awsSns))
    return false;
  if (schema.providerName === 'messageBird'&& !isEmpty(schema.messageBird)) {
      return false;
  }
  return !(schema.providerName === 'clickSend' && !isEmpty(schema.clickSend));
    ...
    addAlert({
      ...
      onDecision: (cancel) => {
        if (!cancel) {
          ...
          patchSmsSettings(updatedSettings).then(
            (res) => {
              dismiss();
              toast({
                title: 'SMS',
        ...
        <div className='space-y-0.5'>
          ...
          <div className={'pr-2'}>
            <p className={'text-xs text-[#94A3B8]'}>
              To  an idea on how to setup your SMS provider take a look at the documentation.
            </p>
          </div>
        </div>
        {smsModule &&
