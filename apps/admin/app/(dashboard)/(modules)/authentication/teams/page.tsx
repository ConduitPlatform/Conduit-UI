...
export default async function Teams({ searchParams }: {
  ...
  const data = await getTeams(skip ?? 0, limit ?? 20, queryParams).catch(() => null);
  if(!data) return (<div>Teams feature inactive go to Authentication Settings to activate</div>);
