...
export default async function TokensPage({ searchParams }: {
  ...
  const data = await getTokens(searchParams.skip ?? 0, searchParams.limit ?? 20, { ...searchParams });
  const refreshTokens = async (search: string) => {
    'use server';
    const {
      tokens,
    } = await getTokens(searchParams.skip ?? 0, searchParams.limit ?? 20, { ...searchParams });
    return tokens;
  };

  return <NotificationTokensTable data={data.tokens} count={data.count}
