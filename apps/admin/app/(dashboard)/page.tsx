...
import { Database, FunctionSquare, HardDrive, ListIcon, LucideMail, User } from 'lucide-react';
...
const items = [
  ...
  },
  {
    ...
    description: 'Data is the heart of your app. Create a model to get started.',
...
export default function Home() {
  return (
    <main className='flex min-h-screen flex-col items-center justify-between p-24'>
      <Stats stats={[{ name: 'Requests', value: '0', unit: 'per second' }, {
        name: 'Avg latency',
        value: 'N/A',
        unit: 'ms',
      }, { name: 'Users', value: 'N/A' }, { name: 'Active Routes', value: '0' }]} />
      ...
      <div>
        ...
        <ul role='list' className='mt-6 grid grid-cols-2 gap-6 border-b border-t border-border py-6'>
          {items.map((item, itemIdx) => (
            <li key={itemIdx} className='flow-root'>
              <div
                className='relative -m-2 flex items-center space-x-4 rounded-xl p-2 focus-within:ring-2 focus-within:ring-indigo-500 hover:bg-secondary'>
                <div
                  className={cn(
                    item.background,
        ...
        <div className='mt-4 flex'>
          <Link href='https://getconduit.dev/docs/overview/intro'
                className='text-sm font-medium text-indigo-600 hover:text-indigo-500' target='_blank'>
            Or read the docs
            <span aria-hidden='true'> &rarr;</span>
          </Link>
        </div>
