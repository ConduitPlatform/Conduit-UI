...
export const BodyParamsForm = () => {
  const form = useFormContext();
  const { fields, append, remove } = useFieldArray({
    control: form.control,
    name: 'options.http.bodyParams',
  });
      ...
      <div className="max-h-96 space-y-1 overflow-auto">
        {fields.map((field, index) => (
          <div className="flex space-x-2 items-center" key={field.id}>
            <InputField
              fieldName={
                `options.http.bodyParams.${index}.params.name` as const
              }
              label={''}
              placeholder={'Key'}
            />
            <FormField
              control={form.control}
              name={`options.http.bodyParams.${index}.params.type` as const}
              render={({ field }) => (
                <FormItem className="w-full space-y-0.5">
                  <Select
                    onValueChange={value => {
                      ...
                      form.setValue(
                        `options.http.bodyParams.${index}.params.type` as const,
                        value
                      );
                    }}
                    value={field.value}
            ...
            <FormField
              control={form.control}
              name={`options.http.bodyParams.${index}.required` as const}
              render={({ field }) => {
                return (
            ...
            <FormField
              control={form.control}
              name={`options.http.bodyParams.${index}.isArray` as const}
              render={({ field }) => {
                return (
                  <FormItem className="flex items-start space-x-1 space-y-0">
                    <FormControl>
